...
3. **If a config file exists:** Parse the **Target** field. If Target does not include `jira` (e.g. it is `github` only), tell the user that Jira is not configured as a publish target and offer to run the GitHub publish step instead, or to add `jira` to Target in the config. Otherwise use the **Jira** section for project key, component, sprint, base URL, and issue type mapping.
4. **If no config file exists:** Use AskQuestion to prompt the user:

   ```
   AskQuestion({
     "title": "No .publish-settings.md found",
     "questions": [{
       "id": "publish_settings",
       "prompt": "No .publish-settings.md found in this project or your home directory. How should I get publish config (including Jira)?",
       "options": [
         {"id": "create_project", "label": "Create .publish-settings.md in this project"},
         {"id": "create_home", "label": "Create ~/.publish-settings.md in my home directory"},
         {"id": "manual", "label": "Specify Jira fields manually (don't save)"}
       ]
     }]
   })
   ```

   - **"Create in project" or "Create in home":** Ask the user for target (`jira`, `github`, or both) and, for Jira, the required fields (project key, component, sprint, JIRA base URL, issue type mapping). Read this skill's `templates/publish-settings.md` for the default template, set **Target** and fill in the **Jira** section (and optionally **GitHub**), then write to the chosen location. Then use it.
